...
<%@ page import="com.egain.platform.util.TimeInstant"%>

<%
    CallerContext callerContext = new CallerContext(1, "Partition-1", 12, "Test User");
    long[] al = {1003};
    //List<CalendarData> olCalendarMainData = EgainCalendar.getAllDateRanges(callerContext, 1005);
    //List olCalendarMainData = EgainCalendar.getAllDateRanges(callerContext, 1005);
    List<String> ad = DayLabel.calendarsUsingDayLabelAsNormalDays(callerContext, al);
%>
...
<body>
    <%
        try{
            //long id = 1022;
            //List<CalendarData> dateRanges = EgainCalendar.getAllDateRanges(callerContext, id);
    %>
        <p><%=ad%></p>
    <%
        } catch (Exception ex){
